...
- Импорт прокси из внешних источников (URL или файл)
- Автоопределение форматов JSON и tg://proxy
- Устойчивый парсер для грязных данных (эмодзи, Markdown-артефакты, переносы строк)
...
2. **Импорт** - добавить прокси из внешнего URL или локального файла (.txt, .json или любой текст с tg://proxy ссылками)
3. **Начать проверку** - запустить ping всех прокси
4. Используйте фильтры для отбора нужных прокси
5. Двойной клик по прокси = QR-код для Telegram
6. **Сохранить** - экспорт в `proxy_results.json`
...
Подробную информацию о политике безопасности см. в [SECURITY_RU.md](SECURITY_RU.md)
