...
- `checker_gui_final.py` - основное приложение с поддержкой локализации
...
5. Добавьте язык в выпадающий список в `checker_gui_final.py`:
...
**Примечание**: Для полного применения языка рекомендуется перезапустить приложение после смены языка.
